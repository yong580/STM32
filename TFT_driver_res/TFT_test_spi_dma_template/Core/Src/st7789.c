...
void ST7789_Fill_Color(uint16_t color) {
  ...
  for (i = 0; i < (ST7789_HEIGHT / HOR_LEN); i++) {
    memset(disp_buf, color, sizeof(disp_buf));  // TODO:自己实现memset
...
void ST7789_Test(void) {
  ...
  //   extern const uint8_t ll_img[];
  ST7789_Fill_Color(WHITE);
  ST7789_DrawImage(0, 0, 240, 240, (uint16_t *)gImage_sunsetcode);
  HAL_Delay(3000);
}
